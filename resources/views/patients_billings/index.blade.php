        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                @if (auth()->user()->isStaff())
                    <a href="{{ route('patient-billing.create') }}"
                    ...
                    <tbody>
                        @forelse ($bills as $bill)
                            <tr class="border-b">
                                ...
                                <td class="px-4 py-2">{{ $bill->stay?->patient?->full_name ?? '—' }}</td>
                                ...
                                <td class="px-4 py-2 text-right space-x-2">
                                    <a href="{{ route('patient-billing.show', $bill) }}" class="text-indigo-600 hover:underline">View</a>
                                    @if (auth()->user()->isStaff())
                                        <a href="{{ route('patient-billing.edit', $bill) }}" class="text-blue-600 hover:underline">Edit</a>
                                        <form action="{{ route('patient-billing.destroy', $bill) }}" method="POST" class="inline" onsubmit="return confirm('Delete this bill?')">
                                            @csrf @method('DELETE')
